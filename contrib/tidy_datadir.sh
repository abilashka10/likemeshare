...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Likemeeeshare database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Likemeeeshare datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Likemeeeshare datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Likemeeeshare 0.7 datadir."
    ;;
  3)
    echo "Detected Likemeeeshare pre-0.8 datadir."
    ;;
  4)
    echo "Detected Likemeeeshare 0.8 datadir."
    ;;
esac
